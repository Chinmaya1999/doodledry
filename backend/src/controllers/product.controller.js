...
// Column order mirrors the "Create Inventory Product" form exactly, so filling
// the sheet feels like the same form, just repeated across many rows.
...
async function loadUploadedWorkbook(file) {
...
export const bulkCreateProducts = asyncHandler(async (req, res) => {
  if (!req.file) throw ApiError.badRequest('Please attach an .xlsx or .csv file.');

  let workbook;
  try {
    workbook = await loadUploadedWorkbook(req.file);
  } catch {
    throw ApiError.badRequest('Could not read this file. Please make sure it is a valid .xlsx or .csv file — ideally the downloaded template.');
  }
  ...
  const requiredCols = ['ageGroup', 'design', 'productType', 'color', 'openingStock', 'reorderLevel', 'costPrice', 'sellingPrice'];
  if (requiredCols.some((key) => !cols[key])) {
    throw ApiError.badRequest('The file must have "Age Group", "Design", "Product Type", "Color", "Opening Stock", "Reorder Level", "Cost Price" and "Selling Price" columns. Download the template and keep its headers.');
  }
